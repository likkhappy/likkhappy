...
// Avoid conflicting when `sea.js` is loaded multiple times
    if (global.seajs && global.seajs.version) {
        return
    }
...
//  - https://developer.mozilla.org/en/HTML/Element/link#Stylesheet_load_events
    ...
    /**
     * module.js - The core of module seajs
     */
...
// For Developers
    ...
    /**
     * config.js - The configuration for the seajs
     */
...
(function(seajs){
    seajs.on("config",function(conf){
        ...
        if('modules' in data){
            ...
            for(var i = 0,len = modules.length; i < len; i++){
                ...
                var version = item[1]
                if( alias != 'seajs' ){
                    alias[id] = data.base + id + '/' + version + '/' + id + '.js'
                }
            }
            delete data.modules;
        }
    ...
    if(cache.config){
        ...
        for(var i = 0,len = config.length;i < len;i++ ){
            seajs.config.apply(this,config[i]);
        }
        delete  cache.config;
    }
    for(var key in cache){
        ...
        for(var i = 0,len = args.length;i < len;i++ ){
            seajs[key]&&seajs[key].apply(this,args[i]);
        }
    }
    delete seajs._cache;
})(seajs);
...
//seajs.use(['seajs/plugin/text'],function(){});
